...
advisory. Reuses the same primitives as verify_acas_contracts.py's discrete
mode -- built for resolving the small number of contracts that
classify_contracts_by_reachability.py flags as "reachable_dangerous_state"
(the UNSAT is not explained by unreachability alone, so the exact reachable
state needs direct verification).
...
from pipeline.neuro.crown.crown_verification import build_crown_config, run_crown_verification
from generate_acas_contracts import A_PREV_TO_NN, ADV_IDX, compute_nn_inputs


def main() -> None:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--advisory",  required=True, choices=A_PREV_TO_NN, help="a_prev -- selects the network")
    p.add_argument("--forbidden", required=True, choices=ADV_IDX,      help="forbidden output advisory")
    ...
    exact = compute_nn_inputs(args.x_mag, args.y_mag, args.x_sign, args.y_sign, args.heading)

    crown_config = build_crown_config(timeout=args.timeout, pgd_order="before", device="cuda")
    status, result = run_crown_verification(
        onnx_path=onnx_path,
        lower=exact,
        upper=exact,
        forbidden_idx=ADV_IDX[args.forbidden],
        num_classes=len(ADV_IDX),
        crown_config=crown_config,
    )

    print(f"state=(x_mag={args.x_mag}, y_mag={args.y_mag}, x_sign={args.x_sign}, "
